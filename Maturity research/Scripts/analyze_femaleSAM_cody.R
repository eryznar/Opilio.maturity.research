...
SAM.dat <- read.csv("./Maturity research/Data/SNOW_femaleSAM_k5.csv") %>%
                   dplyr::select(!X) %>%
                   filter(YEAR >=1989) 
...
t_occ <- read.csv("./Maturity research/Data/BT_occupied_females_cody.csv") %>%
...
write.csv(fem.model.dat, "./Maturity research/Output/SNOW_female_modeldata_cody.csv")
...
model.dat3 <- fem.model.dat2 %>%
  ...
  mutate(
    ...
    # FEM_TOCC_lag1 = lag(FEM_TOCC, 1),
    FEM_TOCC_lag2 = lag(FEM_TOCC, 2),
    FEM_TOCC_lag2avg2 = lag(FEM_TOCC_avg2, 2),
  ...
  dplyr::select(
    ...
    FEM_TOCC_lag2, FEM_TOCC_lag2avg2
...
mod <- gamm(
  SAM ~ 
    ...
    #s(ICE_lag2, k = 4)+
    s(FEM_TOCC_lag2avg2, k = 4),
  correlation = corAR1(),
  data        = model.dat3,
  family      = gaussian(),
  method = "REML"
)
saveRDS(mod, "./Maturity research/Models/SNOW_femaleSAM_gamm_cody.rda")
...
model.dat3 <- fem.model.dat2 %>%
  ...
  dplyr::select(
    ...
    FEM_MAT_ABUND_lag2,
    FEM_MAT_ABUND_avg2lag1,
    FEM_MAT_ABUND_avg2,
    MALE_LG_ABUND_avg2lag1,
    MALE_LG_ABUND_avg2lag2 ,
    ICE_lag1,
    ICE_avg2lag2,
    #ICE_lag2,
    FEM_TOCC_lag1,
...
saveRDS(mod, "./Maturity research/Models/SNOW_femalepmat5565_gam_cody.rda")
