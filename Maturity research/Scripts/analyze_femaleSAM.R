...
vars    <- names(dat_ccf)[!names(dat_ccf) %in% c("YEAR", "SAM", "PMAT_5565", "type", "MATURE", "IMMATURE", "TOT_CRAB")]
cc_df   <- data.frame()
for (vv in vars) {
  pp <- dat_ccf[[vv]]
...
sm.dat <- smooth_estimates(mod) %>%
  ...
  filter(resp == "FEM_MAT_ABUND_avg2", is.na(value) == FALSE)
...
ggplot(sm.dat, aes(x = value, y = .estimate)) +
  ...
  facet_wrap(
    ...
    labeller = as_labeller(c(
      ...
      "s(FEM_TOCC_avg2)"    = "Temperature occupied (t, t-1)")))+ 
  xlab("Value")+
  geom_rug(data = dat_pr_long,
           aes(x = x),
           inherit.aes = FALSE,
