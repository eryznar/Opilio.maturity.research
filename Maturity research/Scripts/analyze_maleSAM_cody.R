...
s.gam <- gam(SEL ~ s(SIZE_5MM), method = "REML", data = sel, family = gaussian(link = "identity"))
...
t_occ <- read.csv("./Maturity research/Data/BT_occupied_cody.csv") %>%
...
saveRDS(mod, "./Maturity research/Models/SNOW_maleSAM_gamm_cody.rda")
...
ggsave("./Maturity research/Figures/SNOW_male_SAM_effectplots_cody.png", width = 8, height = 4)
...
write.csv(indpref.dat, "./Maturity research/Output/SNOW_male_modeldata_cody.csv")
...
saveRDS(mod, "./Maturity research/Models/SNOW_malepmat101_exploitation_gam_cody.rda")
...
## 1. Order data by time and define series starts
df.dat <- df.dat %>%
  arrange(YEAR) %>%                 # or COHORT, or your time index
  mutate(AR_start = c(TRUE, diff(YEAR) != 1))  # TRUE when a new series starts

## 2. Fit an initial BAM *without* AR1 to estimate lag-1 correlation
mod_noAR <- bam(
  cbind(IND_PREF, ALL_MAT - IND_PREF) ~ 
    s(COHORT_ABUND, k = 4) +
    #s(YEAR, bs = "tp")+
  ...
  data    = df.dat,
  family  = quasibinomial(link = "logit"),
  method  = "fREML",    # default for bam()
  discrete = TRUE       # required for non-Gaussian with AR1
)

## 3. Get a starting value for rho from the residual ACF
acf_res <- acf(resid(mod2_noAR, type = "pearson"), plot = FALSE)
rho_start <- acf_res$acf[2]        # lag-1 autocorrelation

rho_start 
# If this is very high (e.g. > 0.8) you might choose something slightly lower, e.g. 0.6–0.7

## 4. Fit the final BAM with AR1
mod_AR <- bam(
  cbind(IND_PREF, ALL_MAT - IND_PREF) ~ 
    s(COHORT_ABUND, k = 4) +
    #s(YEAR, k=4)+
  ...
  discrete = TRUE,
  rho      = rho_start,
  AR.start = df.dat$AR_start
)

saveRDS(mod_AR, "./Maturity research/Models/indpref_updated_AR1.rda")
...
# smooth on a grid (as you already had)
sm.dat <- smooth_estimates(mod_AR) %>%
  tidyr::pivot_longer(cols = 6:ncol(.), names_to = "resp", values_to = "value") %>%
  filter(.smooth == "s(COHORT_ABUND)", !is.na(value))
...
preds <- cbind(p.dat2, pp = predict(mod_AR, p.dat2, type = "response"))
